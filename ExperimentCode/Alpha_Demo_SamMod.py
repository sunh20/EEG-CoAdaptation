...
In this task, subjects close their eyes to move the cursor <>
and keep their eyes open to move the cursor <>

In each trial, the subject attempts to move the cursor into the target by
...
class MyGame(arcade.Window):
    ...
    def create_target(self):
        ...
        if self.trialNum >= len(self.trialType):
            ...
            # Create the target
            im = Image.open('Images/target_2.png')
            img_width, img_height = im.size
            scaling_factor = min(WIDTH/img_width, HEIGHT/img_height)
            self.target_sprite = arcade.Sprite('Images/target_2.png', scale = scaling_factor)
    ...
    def setup(self):
        ...
        # Set up the player
        self.score = 0
        im = Image.open('Images/character_2.png')
        img_width, img_height = im.size
        scaling_factor = min(WIDTH/img_width, HEIGHT/img_height)
        self.player_sprite = Player('Images/character_2.png', scale = scaling_factor)
    ...
    def classify_epoch(self):
        ...
        # Get alpha and beta power for OZ (occipital)
        # closed = move left, open = move right
